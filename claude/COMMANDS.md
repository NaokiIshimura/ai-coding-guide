...
| コマンド | 説明 | 主な機能 |
|---------|------|----------|
| `/codex.implement` | Codexに実装を依頼 | 仕様ファイル読み込み、実装プロンプト生成、Codex実行、結果記録 |
| `/codex.review` | Codexにコードレビューを依頼 | レビュー対象確認、レビュー実行、結果記録、指摘事項サマリー |
| `/codex.refactor` | Codexにリファクタリングを依頼 | 対象ファイル確認、リファクタリング実行、変更内容記録 |
| `/codex.debug` | Codexにデバッグを依頼 | エラー情報収集、原因分析、修正案提示、テスト方法提示 |
...
| コマンド | 説明 | 主な機能 |
|---------|------|----------|
| `/plan.create` | 実行計画（plan.md）を作成 | 実行計画生成、日本時間タイムスタンプ、バージョン管理対応 |
| `/spec.create` | 仕様書（requirements/design/tasks.md）を作成 | 要件定義書・設計書・タスクリスト生成、日本時間タイムスタンプ、バージョン管理対応 |
...
| コマンド | 説明 | 主な機能 |
|---------|------|----------|
| `/memory.write` | memoryファイルに情報を書き込む | 重要情報の保存、MEMORY.md更新、トピック別ファイル管理、引数でディレクトリ指定可 |
| `/memory.read` | memoryファイルを読み込む | MEMORY.md・トピックファイル読み込み、内容の整理・提示、引数でディレクトリ指定可 |
...
| コマンド | 説明 | 主な機能 |
|---------|------|----------|
| `/task.init` | タスク準備を実行 | .claude/plans配下にディレクトリ作成、マークダウンファイル確認・作成 |
...
# 4. 指摘事項を確認して必要に応じて修正（Codexでリファクタリング）
/codex.refactor <ファイルパス>
...
```bash
# 1. エラーが発生した場合、Codexにデバッグを依頼
/codex.debug <エラーメッセージ>
